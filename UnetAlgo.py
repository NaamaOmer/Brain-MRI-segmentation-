...
len(data_folders)  # we have 110 directories

# create images and masks directories to store images and masks
os.mkdir('images')
os.mkdir('masks')

# images and masks directorya
...
def read_and_save_images():
    ...
    for folder in data_folders:  # get each single folder
        ...
        for i in range(1, len(files) // 2):  # get single file
            ...
            mask = Image.open(data_folder_path + '/' + folder + '/' + folder + '_' + str(
                i) + '_mask.tif')  # TCGA_CS_4941_19960909_1_mask.tif
...
print(image.max(), mask.max(), image.min(), mask.min(), image.shape, mask.shape)

IMG_HEIGHT = image.shape[0]
IMG_WIDTH = image.shape[1]
IMG_CHANNELS = image.shape[2]
print(IMG_HEIGHT, IMG_CHANNELS)
# checkpoint
import tensorflow as tf
...
# u-net
# the model:
kernel_initializer = 'he_uniform'  # also try 'he_normal'
...
validation_steps = int(0.1 * num_vals // 8)
print(steps_per_epoch, validation_steps)

# Fit the model
history = model.fit(train_datagen, validation_data=test_datagen, steps_per_epoch=steps_per_epoch,
...
def plot_loss_curves(history_):
    ...
    Args:
      history_: TensorFlow model History object
    ...
    epochs = range(len(history_.history['loss']))
